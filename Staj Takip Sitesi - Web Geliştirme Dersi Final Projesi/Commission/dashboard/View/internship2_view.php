...
<head>
  ...
  <title>Komisyon - Staj 2 Öğrencileri</title>
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0">
              <h3 class="mb-0">Staj 2 Öğrenci Listesi</h3>
            </div>
              ...
              <table class="table align-items-center table-flush">
                ...
                <tbody>
                    <?php
                        ...
                        $sql = "SELECT ogrenci_numarasi FROM staj_basvuru WHERE basvuru_turu ='staj2'";
                        $result = mysqli_query($connection, $sql);
                        if(mysqli_num_rows($result) == 0){
                            echo '<tr><td colspan="4"><h4 style="text-align:center;">Başvuru bulunamadı!</h4></td></tr>';
                        }else{
                            $i = 1;
                            while($row = mysqli_fetch_assoc($result)){
                                $student_no = $row['ogrenci_numarasi'];
                                $new_sql = "SELECT * FROM student WHERE ogrenci_okul_no='$student_no' ";
                                $info = mysqli_query($connection, $new_sql);
                                while($new_row = mysqli_fetch_assoc($info)){
                    ?>
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-primary"><a style="color:#fff ;" href="view-internship2-student-info.php?view-student-info=<?php echo $new_row['kullanci_id'];?>">Görüntüle</a></button>
                                        </td>
                                    </tr>
                    <?php
                    $i++;
