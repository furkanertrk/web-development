...
$faculty = '';
$department = '';
$teacher_id = ''; // ID değişkenini başta tanımladık

if(isset($_GET['view-commission'])){

  $get_id = $_GET['view-commission'];
  
  // Doğrudan veritabanı bağlantısı ve sorgusu (Daha güvenilir yöntem)
  $connection = mysqli_connect('localhost','root','','yazgeldb');
  
  // SQL Enjeksiyonuna karşı basit önlem ve sorgu
  $get_id = mysqli_real_escape_string($connection, $get_id);
  $sql = "SELECT * FROM teacher WHERE ogretmen_id = '$get_id'";
  $result = mysqli_query($connection, $sql);

  if(mysqli_num_rows($result) > 0){
    while($row = mysqli_fetch_assoc($result)){
      $teacher_id = $row['ogretmen_id'];
      $fullName = $row['ogretmen_ad_soyad'];
      $tc = $row['ogretmen_tc'];
      $phone_number = $row['ogretmen_tel'];
      $email = $row['ogretmen_mail'];
      $university_no = $row['ogretmen_okul_no'];
      $password = $row['ogretmen_password'];
      $faculty = $row['ogretmen_fakulte_adi'];
      $department = $row['ogretmen_bolum_adi'];
    }
  }
}
?>
...
<head>
  ...
  <title>Yönetici - Komisyon Bilgileri</title>
...
<body class="">
  ...
  <div class="main-content">
    ...
    <div class="header pb-8 pt-5 pt-lg-8 d-flex align-items-center" style="min-height: 200px; background-color: #0F203F; border-bottom: 4px solid #D4AF37;">
        <div class="container">
           <center><h1 class="new-font mb-4 text-white">Komisyon Bilgileri <i style="color: #D4AF37;" class="fas fa-info-circle"></i></h1></center>
        </div>
    </div>
    ...
    <div class="container-fluid mt--7">
        <div class="order-xl-1">
          <div class="card bg-secondary shadow">
            <div class="card-header bg-white border-0"><div class="row align-items-center"><div class="col-8"><h3 class="mb-0">Hesap</h3></div></div></div>
            <div class="card-body">
            <form method="POST" action="process/crud_commission.php">
                <h6 class="heading-small text-muted mb-4">Komisyon Bilgileri</h6>
                <div class="pl-lg-4">
                  <div class="row">
                    ...
                    <div class="col-lg-6"><div class="form-group"><label class="form-control-label">T.C Kimlik</label><input type="text" name="tc" class="form-control form-control-alternative" value="<?php echo $tc ?>" required></div></div>
                    <div class="col-lg-6"><div class="form-group"><label class="form-control-label">Öğretmen NO</label><input type="text" name="teacher_number" class="form-control form-control-alternative" value="<?php echo $university_no ?>" required></div></div>
                    <div class="col-lg-6"><div class="form-group"><label class="form-control-label">Tel</label><input type="text" name="phone_number" class="form-control form-control-alternative" value="<?php echo $phone_number ?>" required></div></div>
                  ...
                  <div class="row">
                    <div class="col-lg-12">
                      ...
                      <button type="submit" name="remove_from_commission" value="<?php echo $teacher_id; ?>" class="btn btn-warning" onclick="return confirm('Bu kişinin komisyon yetkisini iptal etmek istediğinize emin misiniz?');">Komisyon Üyesi'inden Çıkart</button>
