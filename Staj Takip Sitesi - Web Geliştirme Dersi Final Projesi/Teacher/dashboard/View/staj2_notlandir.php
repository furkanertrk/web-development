<?php 
  ...
  if(isset($_POST['staj-basarili-btn'])){
    ...
    // staj_tur = 'staj2' olarak ayarlandı
    $sql = "UPDATE staj_takibi SET staj_durumu='done', geri_bildirim='$feedback', ogretmen_numarasi='$teacher_number' WHERE ogrenci_numarasi='$student_number' AND staj_tur='staj2'";
  ...
  if(isset($_POST['staj-basarisiz-btn'])){
    ...
    $connection = mysqli_connect('localhost','root','','yazgeldb');
    $sql = "UPDATE staj_takibi SET staj_durumu='basarisiz', geri_bildirim='$feedback', ogretmen_numarasi='$teacher_number' WHERE ogrenci_numarasi='$student_number' AND staj_tur='staj2'";
    mysqli_query($connection, $sql);
    header('Location: internship2_view.php');
    exit();
  }
...
<head>
  ...
  <title>Öğretmen - Staj 2 Notlandırma</title>
...
<body class="">
  ...
  <div class="main-content">
    ...
    <div class="header pb-4 pt-2 pt-lg-8 d-flex align-items-center" style="min-height: 200px; background-color: #0F203F; border-bottom: 4px solid #D4AF37;">
        <div class="container">
        <center>
          <h1 class="new-font mb-4 text-white">Staj 2 Notlandırma <i style="color: #D4AF37;" class="fas fa-edit"></i></h1>
        </center>
        <div class="card shadow">
        <div class="card-body">
            <h3 class="text-center">Notlandırma İşlemi İçin "Görüntüle" Sayfasını Kullanınız.</h3>
            <center><a href="internship2_view.php" class="btn btn-primary">Geri Dön</a></center>
        </div>
        </div>
        </div>
    </div>
