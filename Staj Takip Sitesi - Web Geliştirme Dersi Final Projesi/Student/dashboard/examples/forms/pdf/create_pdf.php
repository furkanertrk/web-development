...
// Türkçe karakterleri PDF için İngilizce karşılıklarına çeviren fonksiyon
...
if(isset($_POST['create_pdf'])){
    ...
    // 1. ADIM: VERİTABANI BAĞLANTISI VE KAYIT İŞLEMİ
    $connection = mysqli_connect('localhost', 'root', '', 'yazgeldb');
    mysqli_set_charset($connection, "utf8"); // Türkçe karakter sorunu olmasın diye

    // Formdan gelen HAM veriler (Veritabanı için)
    ...
    // Staj başvurusu daha önce yapılmış mı kontrol et (İsteğe bağlı, mükerrer kaydı önler)
    $check_sql = "SELECT * FROM staj_basvuru WHERE ogrenci_numarasi='$db_ogrenci_no' AND basvuru_turu='staj1'";
    ...
    if(mysqli_num_rows($check_res) > 0){
        ...
        $sql = "UPDATE staj_basvuru SET 
                ...
                WHERE ogrenci_numarasi='$db_ogrenci_no' AND basvuru_turu='staj1'";
    } else {
    ...
    // Ayrıca staj_takibi tablosuna da durum ekleyelim (eğer yoksa)
    $takip_sql = "SELECT * FROM staj_takibi WHERE ogrenci_numarasi='$db_ogrenci_no' AND staj_tur='staj1'";
    $takip_res = mysqli_query($connection, $takip_sql);
    if(mysqli_num_rows($takip_res) == 0){
        $insert_takip = "INSERT INTO staj_takibi (ogrenci_numarasi, staj_tur, staj_durumu, geri_bildirim) VALUES ('$db_ogrenci_no', 'staj1', 'degerlendirme', 'Yeni Basvuru')";
        mysqli_query($connection, $insert_takip);
    }


    // 2. ADIM: PDF OLUŞTURMA İŞLEMLERİ (Dönüştürülmüş Verilerle)
    ...
    $pdf->Cell(50, 8, 'Adi Soyadi:', 0, 0);
    $pdf->Cell(0, 8, $ad_soyad, 0, 1);
    
    $pdf->Cell(50, 8, 'T.C. Kimlik No:', 0, 0);
    $pdf->Cell(0, 8, $tc, 0, 1);
    
    $pdf->Cell(50, 8, 'Telefon:', 0, 0);
    $pdf->Cell(0, 8, $tel, 0, 1);
    
    $pdf->Cell(50, 8, 'E-posta:', 0, 0);
    $pdf->Cell(0, 8, $email, 0, 1);
    
    $pdf->Cell(50, 8, 'Adres:', 0, 0);
    $pdf->MultiCell(0, 8, $adres);
    
    $pdf->Ln(5);

    // -- STAJ BİLGİLERİ --
    $pdf->SetFont('Helvetica', 'B', 11);
    $pdf->Cell(0, 10, 'STAJ BILGILERI', 0, 1, 'L');
    $pdf->Line(10, $pdf->GetY(), 200, $pdf->GetY());
    $pdf->SetFont('Helvetica', '', 10);
    
    $pdf->Cell(50, 8, 'Baslama - Bitis:', 0, 0);
    $pdf->Cell(0, 8, $baslama . " - " . $bitis, 0, 1);
    
    $pdf->Cell(50, 8, 'Is Gunu / Cumartesi:', 0, 0);
    $pdf->Cell(0, 8, $is_gunu . " Gun / Calisiyor mu: " . $cumartesi, 0, 1);
    
    $pdf->Cell(50, 8, 'Saglik Hizmeti:', 0, 0);
    $pdf->Cell(0, 8, $saglik, 0, 1);
    ...
    $pdf->Cell(50, 8, 'Kurum Adi:', 0, 0);
    $pdf->Cell(0, 8, $kurum_adi, 0, 1);
    
    $pdf->Cell(50, 8, 'Faaliyet Alani:', 0, 0);
    $pdf->Cell(0, 8, $faaliyet, 0, 1);
    
    $pdf->Cell(50, 8, 'Adres:', 0, 0);
    $pdf->MultiCell(0, 8, $kurum_adres);
    
    $pdf->Cell(50, 8, 'Iletisim (Tel/Mail):', 0, 0);
    $pdf->Cell(0, 8, $kurum_tel . " / " . $kurum_mail, 0, 1);
    
    $pdf->Cell(50, 8, 'Devlet Katkisi Istiyor mu:', 0, 0);
    $pdf->Cell(0, 8, $devlet_katkisi, 0, 1);
    ...
    // Dosyayı indir
    $pdf->Output('D', 'Staj_Basvuru_Formu.pdf');
} else {
